...
export default {
	...
	created(){
		...
		.then(response => this.skills = response.body.resource
		, response => M.toast({ html: "Ocorreu um erro ao carregar as Habilidades", classes: "red" }))
  	},
	...
	methods: {
		submit() {
			...
			.then(response => this.skills.push(response.body.resource)
