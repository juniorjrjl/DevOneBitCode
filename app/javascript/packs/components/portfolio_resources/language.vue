...
export default {
	...
	created(){
		...
		.then(response => this.languages = response.body.resource
		, response => M.toast({ html: "Ocorreu um erro ao carregar as Linguagens", classes: "red" }))
	},
	...
	methods: {
		submit() {
			...
			.then(response => this.languages.push(response.body.resource)
